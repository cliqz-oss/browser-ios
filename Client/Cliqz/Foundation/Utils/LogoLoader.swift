...
class LogoLoader {
	...
	private class func hostPrefix(_ hostName: String) -> String {
		let prefixDefaultLength = 2
		let prefixLength = hostName.characters.count >= prefixDefaultLength ? prefixDefaultLength : 0
		return hostName.substring(to: hostName.index(hostName.startIndex, offsetBy: prefixLength))
	}
	...
	private class func fetchLogoDetails(_ url: String) -> LogoInfo {
		...
		if let urlDetails = URLParser.getURLDetails(url),
			...
			logoDetails.prefix = LogoLoader.hostPrefix(hostName).capitalized
