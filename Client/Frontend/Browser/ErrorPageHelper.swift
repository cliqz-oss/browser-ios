    ...
    class func register(_ server: WebServer, certStore: CertStore?) {
        ...
        server.registerHandlerForMethod("GET", module: "errors", resource: "error.html", handler: { (request) -> GCDWebServerResponse! in
            guard let requestURL = request?.url,
				let url = ErrorPageHelper.originalURLFromQuery(requestURL) else {
...
//    func showPage(error: NSError, forUrl url: NSURL, inWebView webView: WKWebView) {
    func showPage(_ error: NSError, forUrl url: URL, inWebView webView: UIWebView) {
        ...
        webView.loadRequest(PrivilegedRequest(url: components.url!) as URLRequest)
    }
...
extension ErrorPageHelper: TabHelper {
    ...
    func userContentController(_ userContentController: WKUserContentController, didReceiveScriptMessage message: WKScriptMessage) {
        if let errorURL = message.frameInfo.request.url, ErrorPageHelper.isErrorPageURL(errorURL),
            ...
            case ErrorPageHelper.MessageCertVisitOnce:
                if let cert = certFromErrorURL(errorURL),
                   let host = originalURL.host {
                    let origin = "\(host):\((originalURL as URL).port ?? 443)"
