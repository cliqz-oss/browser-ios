...
extension SQLiteBookmarks {
    ...
    func getChildrenWithParent(_ parentGUID: GUID, direction: Direction, excludingGUIDs: [GUID]?=nil, includeIcon: Bool) -> Deferred<Maybe<Cursor<BookmarkNode>>> {
        ...
        precondition((excludingGUIDs?.count ?? 0) < 100, "Sanity bound for the number of GUIDs we can exclude.")
...
open class MergedSQLiteBookmarks: BookmarksModelFactorySource {
...
extension SQLiteBookmarksModelFactory {
	...
	fileprivate func cliqzFolder(_ result: Deferred<Maybe<Cursor<BookmarkNode>>>) -> Deferred<Maybe<BookmarkFolder>> {
...
extension BookmarkFactory {
	...
	fileprivate class func cliqzItemFactory(_ row: SDRow) -> CliqzBookmarkItem {
		let id = row["id"] as! Int
		let guid = row["guid"] as! String
		let url = row["bmkUri"] as! String
		let title = row["title"] as? String ?? url
