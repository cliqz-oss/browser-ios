...
extension DataRequest {
    public func responsePartialParsedJSON(_ completionHandler: @escaping (DataResponse<JSON>) -> ()) -> Self {
        return response(responseSerializer: parsedJSONResponseSerializer(), completionHandler: completionHandler)
    }
    
    public func responsePartialParsedJSON(queue: DispatchQueue, completionHandler: @escaping  (DataResponse<JSON>) -> ()) -> Self {
        return response(queue: queue, responseSerializer: partialParsedJSONResponseSerializer(), completionHandler: completionHandler)
    }
    
    public func responseParsedJSON(_ partial: Bool, completionHandler: @escaping (DataResponse<JSON>) -> ()) -> Self {
        return response(responseSerializer: parsedJSONResponseSerializer(), completionHandler: completionHandler)
    }
    
    public func responseParsedJSON(queue: DispatchQueue, completionHandler: @escaping  (DataResponse<JSON>) -> ()) -> Self {
