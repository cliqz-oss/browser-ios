    ...
    public static func fromJSON(_ json: JSON) -> POSTResult? {
        ...
        if let s = json["success"].array,
           let f = json["failed"].dictionary {
            var failed = false
            let asStringOrFail: (JSON) -> String = { $0.string ?? { failed = true; return "" }() }
...
// public typealias ResponseHandler = (NSURLRequest?, NSHTTPURLResponse?, Result<AnyObject, Error>) -> Void
...
open class Sync15CollectionClient<T: CleartextPayloadJSON> {
    ...
    open func post(_ records: [Record<T>], ifUnmodifiedSince: Timestamp?) -> Deferred<Maybe<StorageResponse<POSTResult>>> {
